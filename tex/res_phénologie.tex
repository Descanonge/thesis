...
Similairement à cette étude, nous cherchions à regarder la distribution de la dérivée de \as{chl} par rapport au jour d'inversion du flux de chaleur, et ensuite voir s'il y avait une différence dans ces distributions dans les fronts et dans le background.
Afin d'éviter des erreurs dues à la propagation du bloom, nous réalisions ces diagnostics dans des boîtes de \qtyproduct{5x5}{\degree}.
Les résultats obtenus ne présentaient pas de synchronicité significative entre l'inversion du flux de chaleur et la dérivée de \as{chl} (\nref{fig:inv-hf}).
