...
In addition to these large-scale patterns, there has been considerable evidence over past years that submesoscale motions actively influence the nutrient and light environments at the horizontal scale of the order of 1--50km \parencite[see reviews by][]{levy_2012, mahadevan_2016, levy_2018}.
...
\textcite{keerthi_2022} proposed an approach based on deconvolution of local \as{chl} time series into different timescales; they observed that sub-seasonal time scales contributed roughly 30\% of the total satellite \as{chl} variance and were associated with small (\textless\ 100km) spacial scales.
...
Finally, the contribution of fronts will depend on their spatio-temporal footprint, which also varies seasonally \parencite{callies_2015} and regionally \citep{mauzole_2022}.
...
For \as{chl}, we used the L3 product distributed by ACRI-ST over the period 2000--2020, generated by Copernicus-GlobColour, constructed with data from different sensors (SeaWIFS, MODIS Aqua \& Terra, MERIS, VIIRS-SNPP \& JPSS1, OLCI-S3A \& S3B) merged and reprocessed, available daily at 4km resolution \parencite{chl}.

For \as{sst}, we used the European Space Agency Sea Surface Temperature Climate Change Initiative analysis product version 2.1 \parencite{merchant_2019, good_2020, sst}, also available daily at 4km resolution over the period 2000--2020.
...
The permanently oligotrophic regime to the south of our study area, characterized by warm waters and low \as{chl} (Fig.~\ref{fig:zone-separation}), is known as the subtropical gyre permanently stratified biome \parencite{sarmiento_2004} or permanent deep \as{chl} maximum biome \citep{bock_2022}.
...
More precisely, on each pixel and for each time step, we compute an Heterogeneity Index~(\as{hi}) which quantifies the heterogeneity of the \as{sst} in a square window comprising a few pixels~(\(7 \times 7\)) centered on the pixel of interest.
Thus \as{hi} measures the \as{sst} heterogeneity at spatial scales less than~\qty{30}{\km}.
The \as{hi} is defined as the weighted sum of the skewness~\ab{skew}, standard deviation~\ab{std}, and bimodality~\ab{bimod} of \as{sst} within the window (\(\am{hi} = a \left( b \ab{skew} + c \ab{std} + d \ab{bimod} \right)\), with a, b, c, d constant normalization coefficients).
...
Finally we normalized \as{hi} (coefficient a) such that 95\% of values are below an arbitrary value of 9.5.
...
At each time step, we sorted the pixels into those belonging to strong fronts (defined as \(\mathrm{\as{hi}} > 10\)), those belonging to weak fronts (defined as \(5 < \mathrm{\as{hi}} < 10\)), and those that do not belong to fronts (when \(\mathrm{\as{hi}} < 5\), and called them ``background'' in the following).
...
In each biome, the majority of \as{hi} values are below 5, and as mentioned before, we used this threshold to distinguish pixels in the background from those over fronts (when \(\mathrm{\as{hi}} > 5\)).
...
\begin{figure*}
  ...
  \caption{
    ...
    Note that 0.5\% of pixels have outstanding large \as{hi} values and are not included here.
  }%
  \label{fig:chl-vs-hi}
\end{figure*}
...
With the chosen thresholds, the areal proportion of weak fronts tends to increase from South to North, with 7\%, 19\% and 42\% of \as{hi} values comprised between 5 and 10 in the permanent subtropical, seasonal subtropical, and subpolar biome, respectively.
Regarding strong fronts, there are only present in the seasonal subtropical and subpolar biome (as expected from the fact that the Gulf Stream and shelf-break fronts are located within these biomes), where \as{hi} values above 10 account for 6\% and 17\%, respectively.
...
In order to measure that effect, we computed for each biome the \as{chl} distribution sorted by bins of \as{hi} (of width 0.1) for the whole time series (Fig.~\ref{fig:chl-vs-hi}).
...
The level 4 \as{sst} product used in this study has the advantage to be readily available on download platforms (here CMEMS) at a reasonably high spatial resolution (4km), avoiding the need to regrid as was done in \textcite{liu_2016} who used 1km resolution L2 MODIS-aqua data.
...
We have performed initial tests to investigate the differences in using 1km and 4km resolution \as{sst} data, which convinced us that the use of 4km resolution data was appropriate (not shown), particularly since the heterogeneity index \as{hi} is computed here over boxes of 30km \texttimes\ 30km.
...
In other words, strong fronts, characterized by high values of \as{hi} (\(\mathrm{\as{hi}} > 10\)), led to a stronger increase in \as{chl} values than weak fronts, characterized by intermediate values of \as{hi} (\(5 < \mathrm{\as{hi}} < 10\)).
...
Another element that supports this hypothesis is the scale over which the \as{hi} is computed (30km) which gives a strong weight to \as{sst} heterogeneities associated with large contrasts which is the case across the Gulf Stream.
