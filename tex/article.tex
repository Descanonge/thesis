...
Ocean fronts are narrow zones where horizontal gradients in water properties (temperature, salinity, nutrients, etc.) are significant \citep{belkin_2009a}, and are sometimes described as discontinuities because of their abrupt nature \citep{mauzole_2022}.
...
In the permanent subtropical biome (\nref{fig:latbands-s}), the excess is only detectable north of \latlon{25N} and remains modest (\(\am{excess} < \pct{10}\)); it reaches its maximum of \pct{10} at the beginning and end of the production season.
