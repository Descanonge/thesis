...
We computed the bimodality as the L2 norm of the difference between the SST histogram (with bins of~\tC{0.1}) and a Gaussian fit of the histogram.
...
The level~4 SST product used in this study has the advantage to be readily available on download platforms (here\as{cmems}) at a reasonably high spatial resolution~(\qty{4}{\km}), avoiding the need to regrid as was done in \textcite{liu_2016} who used~\qty{1}{\km} resolution L2 MODIS-Aqua data.
...
We have performed initial tests to investigate the differences in using 1km and 4km resolution SST data, which convinced us that the use of 4km resolution data was appropriate (not shown), particularly since the heterogeneity index HI is computed here over boxes of~\qtyproduct{30 x 30}{\km}.
