...
\begin{note}
  Tout au long de ce manuscript nous utilisons les dénominations des niveaux de données tels que typiquement utilisés par les distributeurs, et qui correspondent de manière générale à:
  \review{TODO}
  \begin{itemize}
    \item \abbrv{L1}:,
    \item \abbrv{L2}:,
...
\footnotetext{%
  ...
  \ab{modis} \slashfrac{s}{}\,Aqua produit des fauchées d'environ~\qty{2300}{\km} de large, qui sont distribuées en sections d'environ la même longueur (le long de la trajectoire).
}
...
\begin{itemize}
  ...
  \item \abbrv{L3U} (mono-capteur mais sur grille régulière),
...
Ces données sont projetées capteur par capteur sur une grille régulière, à une résolution de~\resol{1}{20}, soit~\qty{\approx5}{\km}, ce qui donne le produit~\abbrv{L3U}.
...
On peut néanmoins imaginer que le champ de SST produit est plus lissé dans les zones normalement nuageuses, et de plus potentiellement plus éloigné de la réalité~(voir \cref{sec:donnees-sst_reanalyses}).
...
Les données produites par \review{TODO infos et ref}, disponibles à la résolution~\resol{1}{12}~(\qty{\approx8}{\km})\footfullcite{sst_reanalyses}.
Outre la résolution assez faible de ce produit, on notera des différences importantes avec les autres produits, et qui ne sont visiblement pas due à un simple lissage du champ de SST~(\cref{fig:comparaison-sst}).
...
La version \guil{MultiYear}, au niveau~\abbrv{L3} est utilisée.
...
En effet, il apparaît que la distribution de la SST (au nord de~\latlon{32N}) suffit à repérer de manière fiable et robuste une température seuil permettant de séparer les deux biomes (contour sinueux noir plein~\cref{fig:regions}).
...
Pour ce faire on compare l'écart entre ladite distribution et une distribution gaussienne dont la moyenne et l'écart"-type sont pris identiques à ceux de la SST~(\cref{fig:bimodality}).
...
Les données de SST étant stockées compressées par \engquote{linear packing}\footnotemark\ avec un facteur d'échelle de~\tC{0.01}, nos intervalles ont une largeur précisement égale à dix fois l'intervalle de discrétisation des valeurs de température.
...
\begin{figure}
  ...
  \captionT{Distribution des valeurs de \glsentryshort{hi} et de ses composantes}{%
    ...
    Chacune des composante s'est vue appliquée son coefficient de normalisation, mais pas le HI, qui est donc la simple somme de ces composantes.
  }
...
On peut donc enfin calculer le HI avec:
\begin{equation}
...
Nous avons réalisé des test de sensibilité à certains paramètres de cette méthode \encadra{à savoir la taille de la fenêtre glissante et le choix des coefficients de normalisation} qui sont discutés plus en détail \cref{sec:sensibilite-parametres}.
...
Premièrement, nous détectons la valeur maximale de \as{chl}~(étape\,\textcircled{1}, en orange, \cref{fig:methode-timing-bloom}a).
