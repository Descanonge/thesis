...
Nous savions qu'il allait être nécessaire de combiner plusieurs jeux de données: la SST avec la \ab{chl}, puis avec des données de composition de la communauté de phytoplancton.
...
Nous commençons donc par un jeu de données que l'on désignera par \dataname{sst_modis}, s'appuyant sur les données~\abbrv{L2} du capteur \ab{modis}, à bord du satellite Aqua (\cite{kilpatrick_2015}).
Ce sont les données utilisées par \textcite{liu_2016} \encadra{et produites par les même outils} dont l'étude consistait également à détecter des fronts de SST et les colocaliser aux données de \ab{chl}.

% Éventuellement à déplacer à une mention antérieure des niveaux
\begin{note}[label={note:data-levels}]
  ...
  \par\hspace{2em}\begin{tabularx}{0.9\textwidth}{>{\lfstyle}r X}
    L1 & Données d'un seul capteur, sans traitement (au delà de ceux nécessaire à la transmission depuis le satellite), et à la résolution de capture \encadra*{\ab{cad} projetées dans le référentiel du capteur (nb.\ de lignes et nb.\ de pixels par ligne)}.
    Elles sont accompagnées des données de positionnement du satellite.
    \\
    L2 & Variables géophysiques dérivées des données~\abbrv{L1}, à la même résolution et disposés de la même manière que ces dernières.
    Des métadonnées peuvent les accompagner, comme la qualité des pixels par exemple.
    \\
    L3 & Variables projetées sur une grille régulière en espace et en temps.
    Des traitement supplémentaires peuvent être appliqués.
    \\
    L4 & Résultats d'analyses plus poussées comme l'aggrégation de plusieurs sources de données, ou l'assimilation par un modèle numérique.
  \end{tabularx}
\end{note}
...
Ces données sont projetées sur une grille kilométrique globale, en reprenant les outils utilisés par \citeauthor{liu_2016}, à savoir les programmes présents dans le paquet de l'\href{https://oceandata.sci.gsfc.nasa.gov/ocssw}{\ab{ocssw}}, version~v7.5, qui est maintenu par l'\al{obpg} et notamment distribué à travers l'outil \href{https://seadas.gsfc.nasa.gov/}{SeaDAS}.
...
\footnotetext{%
  ...
  \ab{modis} \slashfrac{s}{}\,Aqua produit des fauchées d'environ~\qty{2300}{\km} de large, qui sont distribuées en sections d'environ la même longueur (\ab{cad} dans la direction de la trajectoire).
}
...
Même pour un capteur identique (\ab{modis} à bord de Terra \ab{par-ex}), il devient nécessaire de considérer des différences de calibrations, ainsi que de possibles artefacts lorsque l'on superpose plusieurs fauchées (que nous avons d'ailleurs largement ignorés pour agréger les fauchées sur une seule journée\dots).
...
Il est également que nous n'avons pas réalisé de détection de nuages supplémentaire et ne disposons que d'un indice de qualité des pixels fourni avec les données~\abbrv{L2}.
Cet indice ne suffit pas à exclure des pixels aux valeurs visiblement erronées.
...
Malgré le fait que l'on soit limité ici à un seul capteur, \ab{modis} présente l'avantage de mesurer concomitamment la SST et la couleur de l'océan.
...
Ce produit est distribué conjointement par l'\ab{esa}"-\glsabbrvfont{SST}"-\ab{cci} et le \ab{c3s}.
Il~agglomère les mesures de tous les capteurs infrarouges disponibles depuis~1981, ce qui comprend 11~\ab{avhrr} à bord des satellites \ab{metop}["~A] et \ab{noaa} (entre les itérations 6 et~19) et trois \ab{atsr} à bord de \ab{ers} 1~et~2, et Envisat. \review{Ajouter SLSTR.}
...
\begin{note}
  ...
  Nous disposerions alors de données~\abbrv{L3} à plus haute résolution, pour un grand ensemble de capteurs (plus nombreux que les deux disponibles pour \ab{modis}).
\end{note}
...
Enfin, le produit~\abbrv{L4} regroupe l'ensemble des capteurs en utilisant le schéma d'intégration variationnelle \abbrv{NEMOVAR} intégré dans le système \ab{ostia} (\cite{good_2020}).
...
Nous utilisons ici une réanalyse globale (désignée par \dataname{sst_reanalyses}), délivrée par \ab{cmems}\footfullcite{sst_reanalyses} et produite par \glshref{mercator}.
...
Le noyau \abbrv{nemo} est forcé en surface par les réanalyses \abbrv{ERA}-Interim et \abbrv{ERA-5} du centre \ab{ecmwf}.
Le modèle fait de l'assimilation de données d'observations: altimétrie, SST \guil{Reynolds}~\resol{1}{4} (seulement \ab{avhrr}), concentration en glace de mer, et profiles in"-situ de températures et profils de salinité~\ab{cora}.
Outre sa résolution assez faible, on notera des différences importantes dans les structures visibles avec les autres produits, et qui ne sont visiblement pas due à un simple lissage du champ de SST~(\nref{fig:comparaison-sst}).

\begin{figure}
  \insertfig{comparaison_sst.pdf}
  \captionT{Comparaison entre produits de SST}{%
    Zoom sur une même fenêtre, le \frenchdate{2007}{04}{22}, de la SST pour quatres produits considérés: \dataname{sst_esacci}~(a), \dataname{sst_reanalyses}~(b), \dataname{sst_mur}~(c), et \dataname{sst_modis}~(d).
...
Sauf indication contraire, c'est le produit~\abbrv{L4} que nous utiliserons pour le champ de SST dans toute la suite de cette thèse.
Il permet d'accéder à des données SST simplement, avec une résolution convenable, et est stable sur une longue durée (plusieurs décennies).
...
La version \engquote{MultiYear} au niveau~\abbrv{L3} est utilisée.
Ce sont des données de résolution spatiale~\resol{1}{24} (soit environ~\qty{4}{\km}), journalières, et qui comportent des nuages.
Elles sont récupérées sur \ab{cmems}\footfullcite{chl_globcolour}.

Ce produit aggrège les données optiques de plusieurs capteurs: \ab{seawifs}, \ab{modis} Aqua et Terra, \ab{meris} à bord d'Envisat, \ab{viirs} à bord de \ab{snpp} et \ab{noaa}[\textlf{-20}], et enfin \ab{olci} à bord de \textlf{Sentinel-3A et -3B}.
...
\begin{figure}
  ...
  \captionT{Comparaison entre produits de Chl-\textit{a}}{%
    \review{TODO}
  }
...
Les deux jeux choisis pour la SST et \ab{chl} ont des résolutions spatiales similaires mais néanmoins légèrement différentes.
Les deux grilles sont plate"-carrées (régulières en latitude et longitude) mais le champ de \ab{chl} est défini sur une grille \glshref{epsg-chl} de résolution~\resol{1}{24}~(\tapprox\qty{4.6}{\km}); et le champ de SST sur une grille \glshref{epsg-sst} de résolution~\resol{1}{20}~(\tapprox\qty{5.6}{\km}).

Sachant que les produits de composition du phytoplancton dont nous disposions (non décrits ici) sont définis sur la même grille que la Chlorophylle, nous adaptons la SST sur la grille de la \ab{chl} par une simple interpolation bi"-linéaire.

\subsection{Bathymétrie}
\label{sec:donnees-bathymetrie}
\declareDataset{etopo1}

Nous utilisons les données de bathymétrie \abbrv{ETOPO1}\footfullcite{etopo1}, fournies par \ab{noaa}.
...
\begin{figure}
  ...
  \captionT{Résultat de la séparation de la zone d'étude en biomes}{%
    ...
    Clichés de SST~(a) et \ab{chl}~(b) au \frenchdate{2007}{04}{22}, et les distributions de SST~(c) et (d)~\ab{chl} pour chacuns des biomes, le même jour (\al{sp}:~bleu, \al{st-sais}:~jaune, \al{st-perm}:~rouge).
...
D'une part la \ab{chl} atteint dans les kilomètres les plus proches de côtes des valeurs très élevées (jusqu'à~\qty{10}{\mgm}) pour des raisons qui ne concernent pas notre étude et qui pourraient fausser les distributions mesurées.
D'autre part, nous cherchons à éviter deux zones qui ne correspondent pas aux biomes définis plus haut.

Premièrement, le jet du Gulf Stream prend forme au sud de notre zone, le long de la côte de Floride. On trouve donc sur le plateau continental à ces latitudes~(\tapprox\latlon{28N}) de forts courants qu'on ne retrouve pas dans le reste du biome \ab{st-perm}.
...
Comme précisé en introduction, pour quantifier l'effets des fronts sur le phytoplancton il est nécessaire de détecter les fronts, \al{cad} de classer chaque pixel comme appartenant à un front ou non (\ab{cad} à l'arrière"-plan ou au \engquote{background}).
...
Enfin, on cherche à quantifier la bimodalité~\ab{bimod} de la distribution des valeurs de SST.
...
\footnotetext{%
  ...
  Par exemple, en prenant pour stockage des entiers non"-signés sur 16~bits~(\texttt{NC\_USHORT}) on peut évidement représenter des valeurs entières entre \num{0} et~\(2^{16}-1 = \num{65535}\); mais en multipliant ces valeurs entières par un facteur de, disons,~\num{0.005} on peut représenter des valeurs entre \num{0} et~\num{327.675}.
...
\begin{figure}
  ...
  \captionT{Exemples de structures fines}{%
    Champs de SST~(colonne gauche), \ab{chl}~(colonne centre), et HI~(colonne droite) pour trois exemples de structures: \textlf{1}\ier~exemple~(1a--c) le \frenchdate{2007}{04}{07}, \textlf{2}\ieme~exemple~(2a--c) le \frenchdate{2007}{02}{23}, et \textlf{3}\ieme~exemple~(3a--c) le \frenchdate{2007}{02}{28}.
...
Le nombre total de pixels étant conséquent, et parce que l'établissement des ensembles est compliqué et coûteux, les ensembles de pixels sont chacun réduits à des histogrammes de variables d'intérêt (SST, \ab{chl}, ou les \abp{pft}).
...
La première est l'excès~\ab{exces} de \ab{chl} (\engquote{excess} dans l'article \nref[section]{sec:article-bg}, \cref*{chp:res-chl}), qui compare localement les valeurs dans les fronts et le background. On le définit simplement comme la différence relative entre la médiane de \ab{chl} dans et hors des fronts:
\begin{equation}
...
On définit donc une deuxième métrique, le surplus~\ab{surplus} de \ab{chl} dans tout un biome (\enquote{biome surplus} dans l'article \nref[section]{sec:article-bg}, \cref*{chp:res-chl}) comme la différence relative entre la moyenne de \ab{chl} dans tout le biome et la moyenne dans le background:
\begin{equation}
...
\begin{figure}
  \centering
  \insertfig{phenologie_2015_N:45-50.pdf}
