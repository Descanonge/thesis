...
Les deux grilles sont plate-carrées (régulières en latitude et longitude) mais le champ de \ab{chl} est défini sur une grille \href{https://epsg.io/32662}{\texttt{EPSG:32662}} de résolution~\resol{1}{24} (\qty{\approx 4.6}{\km}); et le champ de \ab{SST} sur une grille \href{https://epsg.io/4326}{\texttt{EPSG:4326}} de résolution~\resol{1}{20} (\qty{\approx 5.6}{\km}).
...
nécessité de classer pixels des fronts (et non fronts), pour chaque image/jour.
comme vu en intro, plusieurs possibilité, on va se concentrer sur ce qui a été fait pour la submeso.
On reprend l'outil de Liu levine, dont on va donner la définition ici, ainsi que les modifs qu'on y a apporté.

\subsection{Définition et implémentation}
\label{sec:HI-definition}

On rapelle qu'on veut quantifier bimodalité, mais aussi l'écart-type (équivalent au gradient) et le coefficient d'assymétrie (\enquote{skewness}).
...
L'écart-type \ab{std} défini comme:
\begin{equation}
...
Enfin, on cherche à quantifier la bimodalité de la distribution des valeurs de \ab{SST}.
