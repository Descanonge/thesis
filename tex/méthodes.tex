...
Ce produit est distribué conjointement par l'\as{esa}-\abbrv{SST}-\as{cci}\marginnote{\al{cci}} et le \as{c3s}\footnote{%
  l'ESA-CCI pour la période 1981--2016 et le \abbrv{C3S} après 2017
}\footfullcite{sst_esacci}\marginnote{\al{c3s}}.
...
Enfin, le produit~\abbrv{L4} regroupe l'ensemble des capteurs en utilisant le schéma d'intégration variationnelle \abbrv{NEMOVAR} intégré dans le système \as{ostia}\marginnote{Operational SST and Sea Ice Analysis} (\cite{good_2020}).
...
\begin{figure}
  ...
  \captionT{Comparaison des produits de SST}{%
    Zoom sur une même fenêtre, le \frenchdate{2007}{04}{22}, de la SST pour quatres produits considérés: \dataname{sst_esacci}~(a), \dataname{sst_modis}~(b), \dataname{sst_mur}~(c), et \dataname{sst_glorys}~(d).
  }
...
On notera pour le produit de réanalyses \dataname{sst_glorys}~(d), outre sa résolution assez faible, des différences importantes dans les structures présentes sur les autres produits, qui ne sont visiblement pas due à un simple lissage du champ de SST.
...
\begin{note}[label={note:merge-chl-or-rrs}]
  ...
  Aggréger les valeurs de Chlorophylle plutôt que de réflectance permet \encadra{entre autres} un ajout de nouveaux capteurs facilité (à ce jour le produit OC-\as{cci} n'intègre pas encore le capteur de \as{noaa}[-\textlf{20}] par ex.), ainsi qu'une meilleur couverture spatiale~(\cite{garnesson_2019}).
\end{note}
...
\begin{figure}
  ...
  \captionT{Résultat de la séparation de la zone d'étude en biomes}{%
    La zone est découpée en trois biomes: le biome subtropical permanent~(PSB) au sud de du trait pointillé à~\latlon{32N}; le biome subtropical saisonnier~(SSB) entre~\latlon{32N} et la limite (sinueuse) nord du Gulf Stream dénotée par le contour noir; et le biome subpolaire~(PB) au nord du Gulf Stream.
...
\review{TODO: utilisation de flags et d'histogrammes 2D pour le calcul.}
...
{
  \clearpage%
  \makeevenhead{thesis}{\leftmark}{}{Récapitulatif}%
  \makeoddhead{thesis}{\leftmark}{}{Récapitulatif}%
  \phantomsection%
  ...
  \adjustimage{
    center,
    max size={\paperwidth-8cm}{0.99\textheight}
  }{recap_tight.pdf}%
  \vfill
  \clearpage%
}
