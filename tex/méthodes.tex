...
Nous savions qu'il allait être nécessaire de combiner plusieurs jeux de données (\ab{SST} avec \ab{chl}[loaeiflel], puis avec des données de composition de la communauté de phytoplancton).
...
Les données sont téléchargées depuis le \href{cmr.earthdata.nasa.gov/search/}{Common Metadata Repository (CMR)}\footfullcite{sst_modis}, en sélectionnant les swaths intersectant notre région d'étude, collectés de jour, et pour les courtes longueurs d'onde infrarouges (\qty{11}{\um}).
Pour ensuite regridder les données sur une grille kilométrique globale, j'ai repris les outils utilisés par \textcite{liu_2016}, à savoir les programmes présents dans le paquet de l'\href{oceandata.sci.gsfc.nasa.gov/ocssw}{\ab{OCSSW}}, version \verb|v7.5|, qui est maintenu par l'Ocean Biology Processing Group et notamment distribué à travers l'outil \href{https://seadas.gsfc.nasa.gov/}{SeaDAS}.
...
\end{itemize}
La grille utilisée a une résolution de \resol{1}{20} en latitude et longitude (\qty{\sim 5.6}{\km}).
...
% Il n'est plus dispo ou a changé de nom. Le successeur devrait être: \url{data.marine.copernicus.eu/product/GLOBAL_MULTIYEAR_PHY_001_030/description}.
...
Les deux grilles sont plate-carrées (régulières en latitude et longitude) mais le champ de \ab{chl} est défini sur une grille \href{epsg.io/4326}{\texttt{EPSG:4326}} de résolution~\resol{1}{24} (\qty{\sim 4.6}{\km}); et le champ de \ab{SST} sur une grille \href{https://epsg.io/32662}{\texttt{EPSG:32662}} de résolution~\resol{1}{20} (\qty{\sim 5.6}{\km}).
...
La première est l'excès \(E\) de \ab{chl} (\enquote{excess} dans l'article \chapref{sec:article-bg}{chp:res-chl}), qui compare localement les valeurs dans les fronts et le background. On le définit simplement comme la différence relative entre la médiane de \ab{chl} dans et hors des fronts:
\begin{equation}
...
Ainsi on définit le surplus \(S\) de \ab{chl} (\enquote{biome surplus} dans l'article \chapref{sec:article-bg}{chp:res-chl}) dans toute un biome comme la différence relative entre la moyenne de \ab{chl} dans tout un biome et la moyenne dans le background:
\begin{equation}
...
Bien évidemment beaucoup des calculs reposent sur des librairies qui ne sont plus à présenter: \glshref{numpy}, \glshref{scipy}, et \glshref{pandas}.
Cependant il est plus commun (en géosciences en tout cas) d'interfacer avec la librairie \glshref{xarray}.
Les outils de \glshref{dask} sont également avérés indispensables pour gérer les quantités importantes de données que sont les notres.

Il convient de citer plusieurs paquets permettant le calcul efficace d'histogrammes:
\glshref{xhistogram} qui implémente ses fonctionalités à partir de fonctions NumPy et Dask \enquote*{élémentaires}, et
\glshref{dask-histogram} qui s'appuie sur la librairie C Boost.

Plusieurs paquets sont utilisés pour réaliser les figures, bien évidemment \glshref{matplotlib},
\glshref{cartopy} pour les cartes,
mais également \glshref{cmocean} qui fourni des palettes de couleurs, notamment pour un usage en océanographie. Ces dernières présentent les avantages (majeurs) d'être linéairement perceptibles, adaptés à une conversion en nuance de gris pour impression, et robustes à plusieurs types de daltonisme.
Similairement, les coleurs utilisées dans les figures sont le fruit du travail de Paul Tol\footnote{voir \url{personal.sron.nl/~pault/}}, distribué pour Python par mes soins (voir \refunsection{M-sec:productions}).

Enfin il me parait approprié de citer les outils \glshref{ipython} avec lequel j'execute tous mes scripts, \glshref{mamba} et le projet \glshref{conda-forge} lesquels me permettent de gérer les environments Python, et enfin le travail épatant derrière le framework de configuration \glshref{doom} sur lequel tous les scripts et ce manuscript (entre autres) ont été écrits.
...
\begin{itemize}
  ...
  \item \glsitem{xarray}
  \item \glsitem{dask}
  \item \glsitem{xhistogram}
  \item \glsitem{dask-histogram}
  \item \glsitem{boost}
  \item \glsitem{matplotlib}
  \item \glsitem{cartopy}
  \item \glsitem{cmocean}
  \item \glsitem{ipython}
  \item \glsitem{mamba}
  \item \glsitem{conda-forge}
  \item \glsitem{doom}
\end{itemize}

\ifSubfilesClassLoaded{
  \emergencystretch=1em
  \unsection{Bibliographie}
