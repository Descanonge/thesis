...
\begin{note}
  ...
  \par\hspace{2em}\begin{tabularx}{0.9\textwidth}{>{\lfstyle}r X}
    L1 & Données d'un seul capteur, sans traitement (au delà de ceux nécessaire à la transmission depuis le satellite), et à la résolution de capture (donc celle maximale disponible) spatiale et temporelle.
    Elles sont projetées dans le référentiel du capteur (\ab{ie} nombre de lignes et nombre de pixels par ligne), et accompagnées des données de positionnement du satellite.
    \\
...
Ces données sont projetées sur une grille kilométrique globale, en reprenant les outils utilisés par \citeauthor{liu_2016}, à savoir les programmes présents dans le paquet de l'\href{https://oceandata.sci.gsfc.nasa.gov/ocssw}{\ab{ocssw}}, version~v\num{7.5}, qui est maintenu par l'\al{obpg} et notamment distribué à travers l'outil \href{https://seadas.gsfc.nasa.gov/}{SeaDAS}.
...
Les deux grilles sont plate"-carrées (régulières en latitude et longitude) mais le champ de \ab{chl} est défini sur une grille \glshref{epsg-chl} de résolution~\resol{1}{24}~(\qty{\approx4.6}{\km}); et le champ de SST sur une grille \glshref{epsg-sst} de résolution~\resol{1}{20}~(\qty{\approx5.6}{\km}).
