...
Ce produit est distribué conjointement par l'\as{esa}-\abbrv{SST}-\as{cci}\marginnote{\al*{cci}} et le \as{c3s}\footnote{\fullcite{sst_esacci}}\multfootsep\footnote{%
...
Enfin, le produit~\abbrv{L4} regroupe l'ensemble des capteurs en utilisant le schéma d'intégration variationnelle \abbrv{NEMOVAR} intégré dans le système \as{ostia}\marginnote{\al*{ostia}} (\cite{good_2020}).
...
On notera pour le produit de réanalyses \dataname{sst_glorys}, outre sa résolution assez faible, des différences importantes dans les structures présentes sur les autres produits, qui ne sont visiblement pas due à un simple lissage du champ de SST.
