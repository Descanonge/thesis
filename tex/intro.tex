...
\begin{figure}[!h]
  ...
  \captionT{Les couleurs du Gulf Stream par satellite}{%
    ...
    Adapté de l'\eng{Image of the day} \hbox{2020-03-10} \textit{Hints of Spring in the Atlantic}, sur \eng{\glshref{ve-illustration} \& \glshref{eo-illustration}}.
  }
...
\begin{figure}
  ...
  \captionT{La submésoéchelle en photos}{%
    ...
    À droite, une image en fausses couleurs prise par \as{oli} \sur~Landsat"~8, en mer de Barents lors d'un bloom d'été, probablement de cyanobactéries (\eng{\glshref{eo-sms}: image of the day \hbox{2015-08-23}}, image par Norman Kuring).
  }
...
Cette instabilité de type barocline tend à aplanir les isopycnes sur toute la hauteur de la couche de mélange en créant des tourbillons de submésoéchelles.

\begin{note}
  Le mode de croissance maximale de cette instabilité est de taille \(L = \frac{NH}{f}\), avec \(N \simeq \qty{e-3}{s^{-1}}\) la fréquence de Brunt-Väisälä relative à la couche de mélange, \(H \simeq \qty{200}{\meter}\) la hauteur de la couche de mélange, et \(f \simeq \qty{1e-4}{s^{-1}}\) le paramètres de Coriolis aux moyennes latitudes.
...
On commencera par rappeler que les zones d'upwelling et de downwelling sont séparées spatialement, les nutriments remontés ne sont donc pas directement renvoyés en profondeur.
...
\begin{figure}
  ...
  \captionT{Délimitations des biomes par des processus physiques}{%
    Tiré de \textcite{sarmiento_2004}, figure~2b.
    ...
    \foreignblockquote{english}{
      Biome classification scheme calculated using mixed layer depths obtained from observed density and from upwelling calculated from the wind stress divergence using observed winds.
