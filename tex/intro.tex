...
Afin de mesurer l'impact des sub-mesoéchelle sur le phytoplancton, nous partons de l'idée de détecter les fronts de densité à partir de la SST afin de les colocaliser aux valeurs de \al{chl}.
...
Initialement développée pour de la détection de contours en traitement d'image \encadra{domaine où elle demeure un standard, et où ses implémentations sont amplement disponibles}, elle a également été largement appliquée en océanographie à la détection de fronts en température, entre autres (cite{93-101}).
...
Il a été développé afin de cibler à la fois les fronts de \as{chl} et de SST.
...
La méthode \abbrv{CC} a largement été utilisée pour détecter des fronts de SST
(cite(107, 134)). (10.1007/s12524-022-01629-1), notamment dans une étude de la répartition des fronts à l'échelle globale (2).
Elle a également été appliquée avec succès sur des données de Chlorophylle (110, 124, 135).
...
Plutôt que d'utiliser des images à des instants différents, \textcite{nieto_2012} applique l'algorithme \abbrv{CC} sur quatres fenêtre glissantes décalées en longitude et latitude et fusionnent les résultats des pixels se chevauchant.
\textcite{miller_2009} combine les résultats de l'algorithme \abbrv{CC} pour des produits SST et \as{chl} afin d'augmenter la couverture disponible.

Une méthode au fonctionnement similaire est proposé par \textcite{vazquez_1999}, qui pour quantifier la séparation des deux distributions en température (de part et d'autre d'un éventuel front) utilise une mesure entropique: la divergence de Jensen--Shannon (\cite{barranco-lopez_1995}).
...
Enfin, dans le but de quantifier les valeurs de \as{chl} dans les fronts de sub-mesoéchelle de la gyre subtropicale du Pacifique Nord, \textcite{liu_2016} propose une méthode inspiré de l'algorithme \as{cc}.
Les auteur·ices définissent un indice d'hétérogénéité (\as{hi}) du champ de SST.
...
L'indice calculé (\al{hi}) cherche plus à quantifier l'hétérogénéité spatiale de la SST (sa \engquote{patchiness}) pour chaque pixel, qu'à \guil{détecter les fronts}.
La plupart des méthodes introduites plus haut incluent une étape algorithmique (omise ci"-dessus) permettant d'obtenir en sortie uniquement la position des fronts, représentée par une ligne d'épaisseur nulle (ou 1~pixel).
Par opposition à la démarche de \citeauthor{liu_2016}, où les fronts sont définis par un simple\footnotemark seuil sur le HI, ce qui donne une surface de fronts variable.
À l'inverse, l'algorithme \as{cc} par exemple est construit de manière beaucoup plus dichotomique.
...
Dans le \nref[chapitre]{chp:méthode}, nous commencons par détailler les différents ensembles de données que nous avons considérés, et expliciter le
Le \nref[chapitre]{}
