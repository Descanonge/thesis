...
On les combine ensuite en une seule variable \(v\) stockée sur 8~bits (le minimum nécessaire), en commençant par le bit le moins signifiant (par la droite ici): glace, nuage, algorithme, qualité.
...
Une fois cette variable créée, les drapeaux individuels sont récupérés simplement en appliquant un masque binaire, ce qui correspond à une opération \guil*{et} bit à bit (\engquote{bitwise AND}, noté \textsf{\&} pour \citesoft{numpy}).
Par exemple pour récupérer la valeur de qualité d'un pixel on fera l'opération: \(v \& 48 (\overline{00110000}) \).
La valeur récupérée est encore décalée sur la gauche, ici de 4~bits (\as{ie} multipliée par 8), mais dans convention \as{cf} la signification des valeurs est stockée décalée, on aura donc directement 16: qualité~1, 32: qualité~2, 48: qualité~3.
...
Je défini quelques méthodes supplémentaires à une classe d'\emph{Accessor}, ce qui les rendra accessibles à tous les objets XArray concernées (\as{ie} une variable \py{xarray.DataArray} contenant les drapeaux à extraire):
\begin{python}
