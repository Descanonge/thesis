...
\section{Objectifs et plan de la thèse}
...
\cref{chp:méthodes}: Description des méthodes utilisées, notamment pour repérer les fronts.
\cref{chp:res-chl}: Chloro dans la zone du Gulf Stream.
\cref{chp:res-pft}: dans la même zone, mais pour les groupes de phyto.
\cref{chp:global}: Étude des fronts à l'échelle globale.
\cref{chp:conclusion}: Conclusion générale.
Dans le reste de ce premier chapitre: introduction aux problèmatiques que l'on va rencontrer, aux concepts qu'on va utiliser, et aux méthodes pré-existantes.
